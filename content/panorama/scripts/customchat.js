  ...
  if (playerId != null && !Game.IsPlayerMuted(playerId)) {
    ...
    if (playerId !== -1) {
      ...
      if (SenderHero && SenderHero !== 'npc_dota_hero_target_dummy')
        html =
          '<img src="' +
          TransformTextureToPath(SenderHero) +
          '" class="HeroIcon" style="vertical-align: top;"/> ';
      html +=
        data.teamonly === 1 ? '<font color="lime">[T]</font>' : '<font color="darkred">[A]</font>';
      html +=
        " <font color='" +
        playerColor +
        "'>" +
        _.escape(Players.GetPlayerName(playerId)) +
        '</font>: ';
    }
    ...
    if (data.text) {
      ...
      html +=
        "<img src='file://{images}/control_icons/chat_wheel_icon.png' class='ChatWheelIcon' />";
      ...
      localized = localized.replace(
        '{player}',
        '<font color="' +
          GetHEXPlayerColor(data.player) +
          '"">' +
          $.Localize(GetPlayerHeroName(data.player)) +
          '</font>',
      );
      html += localized;
      ...
      var localized =
        data.xpToNextLevel == null && !data.isNeutral
          ? 'chat_message_level_side_capped'
          : 'chat_message_level_side';
      var unitSide =
      ...
      localized = localized.replace(
        '{player}',
        "<font color='" +
          (data.player === -1 ? '#FFFFFF' : GetHEXPlayerColor(data.player)) +
          "'>" +
          $.Localize(GetHeroName(data.unit)) +
          '</font>',
      );
      html += localized;
      ...
      if (data.player != null)
        localized = localized.replace(
          ...
          "<font color='" +
            (data.player === -1 ? '#FFFFFF' : GetHEXPlayerColor(data.player)) +
            "'>" +
            $.Localize(Players.GetPlayerName(data.player)) +
            '</font>',
        );
      html += localized;
    }
...
function AddSmiles(string) {
  return string
    .replace(twitchRegExp, function(matched) {
