  ...
  for (var a in GetLocalAbilityNamesInfo()) {
    ...
    if (abinf != null)
      for (var v in abinf.banned_with)
        if (b.indexOf(abinf.banned_with[v]) === -1) b.push(abinf.banned_with[v]);
  }
  return b;
}
...
function Search() {
  ...
  if (
    ...
    !_.isEqual(SearchingAbLevels, abLevels)
  ) {
    SearchingFor = SearchText;
    SearchingPurchasedChecked = $('#PurchasedAbilitiesToggle').checked;
    ...
    if (ShowSearch) {
        ...
        _.each(AbilityShopData, function(tabContent) {
          _.each(tabContent, function(heroData) {
            ...
            _.each(heroData.abilities, function(abilityData) {
              if (
                (hero.toLowerCase().indexOf(SearchText) !== -1 ||
                  abilityData.ability.toLowerCase().indexOf(SearchText) !== -1 ||
                  $.Localize('#' + hero.toLowerCase())
                    .toLowerCase()
                    .indexOf(SearchText) !== -1 ||
                  $.Localize(abilityData.ability.toLowerCase())
                    .toLowerCase()
                    .indexOf(SearchText) !== -1) &&
                (!SearchingPurchasedChecked ||
                  (SearchingPurchasedChecked && abLevels[abilityData.ability] != null))
              ) {
                FoundAbilities.push({
      ...
      _.each(_.sortBy(FoundAbilities, 'name'), function(abilityInfo) {
        AllAbilityPanels.push(
          CreateSnippet_Ability(
            $.CreatePanel('Panel', ShopSearchOverlay, ''),
            abilityInfo.name,
            abilityInfo.hero,
            abilityInfo.data.cost,
          ),
        );
      });
    }
...
function UpdateAbilities() {
  ...
  _.each(AllAbilityPanels, function(panel) {
    ...
    } else panel.RemoveClass('MaxUpgraded');
    ...
    if (panel.BHasClass('Purchased') && GameUI.IsShiftDown()) {
      ...
      panel.FindChildTraverse('SellCost').text =
        '-' + CalculateDowngradeCost(abilityname, cost) * purchasedInfo.level;
    ...
    } else panel.RemoveClass('CanDelete');
...
function UpdateHeroAbilityList(hero, abilities) {
  ...
  _.each(abilities, function(abilityInfo) {
    AllAbilityPanels.push(
      CreateSnippet_Ability(
        $.CreatePanel('Panel', abilitiesroot, ''),
        abilityInfo.ability,
        hero,
        abilityInfo.cost,
      ),
...
(function() {
  DynamicSubscribePTListener('ability_shop_data', function(
    tableName,
    changesObject,
    deletionsObject,
  ) {
  ...
  (function autoUpdate() {
    Search();
    UpdateAbilities();
    $('#ShiftStateLabel').text = $.Localize(
      GameUI.IsShiftDown() ? '#ability_shop_shift_yes' : '#ability_shop_shift_no',
    );
