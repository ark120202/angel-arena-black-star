...
function UpdateItembuildsForHero() {
  ...
  if (
    LastHero !== heroName &&
    heroName &&
    heroName !== 'npc_dota_hero_arena_base' &&
...
function LoadGuidesForHero(heroName, skipAutoSelect, callback) {
  GetDataFromServer(
    'GetGuides',
    { steamID: Game.GetLocalPlayerInfo().player_steamid, hero: heroName },
    function(builds) {
...
function SelectItembuild(build) {
  ...
  if (build != null) {
    ...
    _.each(build.items, function(groupData) {
      CreateItembuildGroup(
        groupsRoot,
        build.steamID === -1 ? $.Localize(groupData.title) : groupData.title,
        groupData.content,
      );
    });
  } else {
...
function UpdateSelectedItembuildAuthorData(steamID, title) {
  ...
  $('#Itembuild_title').text =
    steamID === 0
      ? $.Localize('DOTA_UI_default_build')
      : steamID === -1
      ? $.Localize('DOTA_UI_scratch_build_for').replace('%s', $.Localize(LastHero))
      : title || '';
...
function CreateItembuildGroup(groupsRoot, title, content) {
  ...
  var createPanelForGroup = function(item) {
    var itemPanel = SnippetCreate_SmallItem(
      $.CreatePanel('Panel', itemsRoot, 'shop_itembuild_items_' + item),
      item,
      false,
      function(panel) {
...
function GuideVote(vote) {
  ...
  GameEvents.SendCustomGameEventToServer('stats_client_vote_guide', {
    id: CurrentPageGuide._id,
    vote: vote,
  });
}
...
function OpenYoutubeForGuide(build) {
  if (build && build.youtube)
    $.DispatchEvent('ExternalBrowserGoToURL', 'https://www.youtube.com/watch?v=' + build.youtube);
}
...
function PublishGuide(build) {
  ...
  if (
    build.title ===
    $.Localize('DOTA_UI_scratch_build_for')
      .replace('%s', $.Localize(LastHero))
      .substring(0, 20)
  ) {
