...
function Snippet_SharedControlPlayer_Update(panel) {
  panel.visible =
    Players.GetTeam(panel.playerId) === Players.GetTeam(Game.GetLocalPlayerID()) &&
    Snippet_Player(panel.playerId).visible;
...
function Snippet_Player(playerId) {
  if (PlayerPanels[playerId] == null) {
    ...
    if (team !== DOTA_TEAM_SPECTATOR) {
      ...
      panel.FindChildTraverse('AvatarImage').SetPanelEvent('onactivate', () => {
        GameEvents.SendEventClientSide('player_profiles_show_info', { playerId });
      });
...
function Snippet_Player_Update(panel) {
  ...
  if (players_abandoned.indexOf(playerId) !== -1) {
    ...
    var ultStateOrTime =
      playerInfo.player_team_id === Players.GetTeam(Game.GetLocalPlayerID())
    ...
    if (
      ...
      panel.SetParent(
        Snippet_Team(playerInfo.player_team_id).FindChildTraverse('TeamPlayersContainer'),
      );
      panel.Resort();
    }
...
function Snippet_Team(team) {
  if (TeamPanels[team] == null) {
    ...
    panel.FindChildTraverse('TeamScoreLabel').style.textShadow =
      '0px 0px 6px 1.0 ' + teamColors[team];
...
(function() {
  ...
  DynamicSubscribePTListener('disable_help_data', function(
    tableName,
    changesObject,
    deletionsObject,
  ) {
    if (changesObject[LocalPlayerID] != null) {
      _.each(changesObject[LocalPlayerID], function(state, playerId) {
        Snippet_SharedControlPlayer(playerId).FindChildTraverse('DisableHelpButton').checked =
          state === 1;
      });
    }
  });
