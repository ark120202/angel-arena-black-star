...
function Snippet_OptionVoting(voteName, voteData) {
  ...
  for (var tIndex in voteData.variants) {
    ...
    var group = shouldGroup
      ? OptionVotingVariants.GetChild(Math.floor((tIndex - 1) / 2)) ||
        $.CreatePanel('Panel', OptionVotingVariants, '')
      : OptionVotingVariants;
...
function Snippet_OptionVoting_Recalculate(votePanel, voteData) {
  var amount = {};
  for (var k in voteData.variants) amount[voteData.variants[k]] = 0;
  ...
  for (var variant in amount) {
    ...
    variantPanel.SetDialogVariable(
      'votes',
      count + ' (' + Math.round((count / (total || 1)) * 100) + '%)',
    );
...
function CheckStartable() {
  ...
  if (player == null) {
    ...
    DynamicSubscribePTListener('option_votings', function(
      tableName,
      changesObject,
      deletionsObject,
    ) {
