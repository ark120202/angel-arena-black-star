...
function Snippet_TopBarPlayerSlot_Update(panel) {
  ...
  if (players_abandoned.indexOf(playerId) !== -1) {
    ...
    panel.SetHasClass(
      'Disconnected',
      connectionState === DOTAConnectionState_t.DOTA_CONNECTION_STATE_DISCONNECTED,
    );
    panel
      .FindChildTraverse('HeroImage')
      .SetImage(TransformTextureToPath(GetPlayerHeroName(playerId)));
    panel.FindChildTraverse('PlayerColor').style.backgroundColor = GetHEXPlayerColor(playerId);
    var ultStateOrTime = isAlly
      ? Game.GetPlayerUltimateStateOrTime(playerId)
    ...
    panel.FindChildTraverse('ManaBar').value =
      Entities.GetMana(heroEnt) / Entities.GetMaxMana(heroEnt);
    ...
    panel.SetHasClass(
      'BuybackUsed',
      lastBuybackTime !== 0 && Game.GetGameTime() - lastBuybackTime < 10,
    );
    if (
      playerInfo.player_team_id !== DOTA_TEAM_SPECTATOR &&
      playerInfo.player_team_id !== panel.GetParent().team &&
      teamColors[playerInfo.player_team_id] != null
    ) {
      panel.SetParent(
        Snippet_DotaTeamBar(playerInfo.player_team_id).FindChildTraverse('TopBarPlayersContainer'),
      );
    }
  }
}
...
function Update() {
  ...
  context.SetDialogVariable(
    'time_until',
    secondsToMS((isDayTime ? 300 : 600) - timeThisDayLasts, true),
  );
  ...
  context.SetDialogVariable(
    'duel_timer',
    secondsToMS(
      DuelTimerEndTime == null
        ? time
        : Math.max(Math.ceil(DuelTimerEndTime - Game.GetGameTime()), 0),
    ),
  );

  $('#WeatherTime').SetDialogVariable(
    'weather_duration',
    secondsToMS(Math.ceil(Game.GetDOTATime(false, true) - changeTime)),
  );
