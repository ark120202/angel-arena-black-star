  ...
  if (panel == null) {
    ...
    if (playerId != -1) {
      ...
      $.RegisterEventHandler('DragEnter', panel, function(a, draggedPanel) {
        if (draggedPanel.playerId == null || draggedPanel.playerId == playerId) return true;
...
function Snippet_Team(team) {
  if (TeamPanels[team] == null) {
    ...
    $.RegisterEventHandler('DragEnter', panel, function(a, draggedPanel) {
      if (draggedPanel.team == null || draggedPanel.team == team) return true;
      panel.AddClass('potential_drop_target');
      return true;
    });
    $.RegisterEventHandler('DragDrop', panel, function(panelId, draggedPanel) {
      panel.RemoveClass('potential_drop_target');
      if (draggedPanel.team == team) return true;
      HostSwapPlayerWithTeam(draggedPanel.playerId, team);
      return true;
    });
    $.RegisterEventHandler('DragLeave', panel, function(panelId, draggedPanel) {
      if (draggedPanel.team == null || draggedPanel.team == team) return false;
      panel.RemoveClass('potential_drop_target');
      return true;
    });
...
function UpdateTimer() {
  ...
  if (playerInfo != null) {
    $.GetContextPanel().SetHasClass(
      'player_has_host_privileges',
      playerInfo.player_has_host_privileges && Options.GetMapInfo().gamemode !== 'ranked',
    );
  }
  if (transitionTime >= 0) {
...
function OnTeamPlayerListChanged() {
  ...
  Game.SetRemainingSetupTime(
    Game.GetTeamSelectionLocked() ? LockTime : unassignedPlayers.length == 0 ? NormalTime : -1,
  );
...
function OnShufflePlayersPressed() {
  ...
  Game.SetRemainingSetupTime(
    Game.GetTeamSelectionLocked()
      ? LockTime
      : Game.GetUnassignedPlayerIDs().length == 0
      ? NormalTime
      : -1,
  );
}
