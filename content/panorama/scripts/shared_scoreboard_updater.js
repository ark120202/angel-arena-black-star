...
function _ScoreboardUpdater_UpdatePlayerPanel(scoreboardConfig, playersContainer, playerId, localPlayerTeamId) {
	...
	if (playerInfo) {
		...
		if (isTeammate) {
			...
			_ScoreboardUpdater_SetTextSafe(playerPanel, "LastHits", Players.GetLastHits(playerId));
		} else
			_ScoreboardUpdater_SetTextSafe(playerPanel, "LastHits", "");
		goldValue = GetPlayerGold(playerId)
		...
		if (playerPortrait) {
			if (HeroName !== "") {
				playerPortrait.SetImage("file://{images}/heroes/" + HeroName + ".png");
			} else {
...
function _ScoreboardUpdater_UpdateTeamPanel(scoreboardConfig, containerPanel, teamDetails, teamsInfo) {
	...
	var AbandonedPlayers = PlayerTables.GetTableValue("arena", "players_abandoned")
	...
	if (playersContainer) {
		...
		for (var i = playersContainer.GetChildCount() - 1; i >= 0; i--) {
			...
			if (teamPlayers.indexOf(playerId) == -1 || (AbandonedPlayers != null && GameUI.CustomUIConfig().IsContains(AbandonedPlayers, playerId))) {
