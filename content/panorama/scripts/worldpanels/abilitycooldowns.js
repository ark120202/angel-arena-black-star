  ...
  if (!offScreen && wp) {
    ...
    if (ent) {
      ...
      for (var i = 0; i < Entities.GetAbilityCount(ent); i++) {
        ...
        if (
          ability !== -1 &&
          !_.startsWith(Abilities.GetAbilityName(ability), 'special_bonus_') &&
      ...
      if (
        !_.isEqual(EntityAbilitiesNames, AbilityBox.EntityAbilitiesNames) ||
        AbilityBox.GetChildCount() <= 0
      ) {
        ...
        for (k in EntityAbilities) {
          ...
          var cooldownOverlayPanel = $.CreatePanel(
            'Panel',
            cooldownPanel,
        ...
        if ($.GetContextPanel().Data.hasHealthBar) {
          ...
          $('#HealthBar').BLoadLayout(
            'file://{resources}/layout/custom_game/worldpanels/healthbar.xml',
            true,
            true,
          );
        }
      ...
      for (var i = 0; i < AbilityBox.GetChildCount(); i++) {
        ...
        abilityPanel.GetChild(1).FindChild('AbilityLevel').text = Abilities.GetLevel(
          abilityPanel.abilityId,
        );

        if (
          abilityPanel.GetChild(1).FindChild('AbilityManacostPanel').visible !=
          !Abilities.IsOwnersManaEnough(abilityPanel.abilityId)
        ) {
          abilityPanel
            .GetChild(1)
            .FindChild('AbilityManacostPanel').visible = !Abilities.IsOwnersManaEnough(
            abilityPanel.abilityId,
          );
        }
        ...
        if (abilityPanel.abilityId > -1 && !Abilities.IsCooldownReady(abilityPanel.abilityId)) {
          ...
          abilityPanel.GetChild(1).FindChild('AbilityCooldownTimer').text = Math.ceil(
            cooldownRemaining,
          );
          abilityPanel.GetChild(1).FindChild('AbilityCooldownOverlay').style.width =
            cooldownPercent + '%';
        } else {
