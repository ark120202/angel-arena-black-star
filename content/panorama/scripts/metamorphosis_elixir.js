...
function CloseMenu() {
  ...
  if (AutoSearchHeroThinker != null) $.CancelScheduled(AutoSearchHeroThinker);
}
...
function UpdateHeroesSelected() {
  for (var i = 0; i < HeroesPanels.length; i++) {
    ...
    heroPanel.SetHasClass(
      'Picked',
      IsHeroPicked(heroPanel.id.substring('HeroListPanel_element_'.length)),
    );
    heroPanel.SetHasClass(
      'Locked',
      IsHeroLocked(heroPanel.id.substring('HeroListPanel_element_'.length)),
    );
  }
