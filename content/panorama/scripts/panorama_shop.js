  ...
  if (SearchingFor !== searchStr) {
    ...
    if (searchStr.length > 0) {
      ...
      var foundItems = _.filter(_.keys(ItemData), function(itemName) {
        ...
        return (
          !_.startsWith(itemName, 'item_recipe_') &&
      ...
      _.each(foundItems, function(itemName) {
        SnippetCreate_SmallItem(
          $.CreatePanel('Panel', ShopSearchOverlay, 'ShopSearchOverlay_item_' + itemName),
          itemName,
        );
      });
    }
...
function SnippetCreate_SmallItem(panel, itemName, skipPush, onDragStart, onDragEnd) {
  ...
  panel.DestroyItemPanel = function() {
    ...
    if (id1 > -1) SmallItemsAlwaysUpdated.splice(id1, 1);
    if (id2 > -1) SmallItems.splice(id2, 1);
    panel.visible = false;
    panel.DeleteAsync(0);
  };
  ...
  if (!skipPush) SmallItems.push(panel);
  return panel;
}
...
function ShowItemRecipe(itemName) {
  ...
  _.each($('#ItemRecipeBoxRow1').Children(), function(child) {
    if (child.DestroyItemPanel != null) child.DestroyItemPanel();
    else child.DeleteAsync(0);
  });
  ...
  var itemPanel = $.CreatePanel(
    'Panel',
    $('#ItemRecipeBoxRow2'),
    'ItemRecipeBoxRow2_item_' + itemName,
  );
  SnippetCreate_SmallItem(itemPanel, itemName);
  ...
  if (RecipeData != null && RecipeData.items != null) {
    _.each(RecipeData.items[1], function(childName) {
      var itemPanel = $.CreatePanel(
        'Panel',
        $('#ItemRecipeBoxRow3'),
        'ItemRecipeBoxRow3_item_' + childName,
      );
    ...
    if (RecipeData.visible && RecipeData.recipeItemName != null) {
      ...
      var itemPanel = $.CreatePanel(
        'Panel',
        $('#ItemRecipeBoxRow3'),
        'ItemRecipeBoxRow3_item_' + RecipeData.recipeItemName,
      );
  ...
  if (BuildsIntoData != null) {
    _.each(BuildsIntoData, function(childName) {
      var itemPanel = $.CreatePanel(
        'Panel',
        $('#ItemRecipeBoxRow1'),
        'ItemRecipeBoxRow1_item_' + childName,
      );
      SnippetCreate_SmallItem(itemPanel, childName);
      itemPanel.style.align = 'center center';
    });
    $('#ItemRecipeBoxRow1').SetHasClass(
      'ItemRecipeBoxRowLength7',
      Object.keys(BuildsIntoData).length >= 7,
    );
    $('#ItemRecipeBoxRow1').SetHasClass(
      'ItemRecipeBoxRowLength8',
      Object.keys(BuildsIntoData).length >= 8,
    );
    $('#ItemRecipeBoxRow1').SetHasClass(
      'ItemRecipeBoxRowLength9',
      Object.keys(BuildsIntoData).length >= 9,
    );
  }
}
...
function SendItemBuyOrder(itemName) {
  ...
  unit = Entities.IsControllableByPlayer(unit, playerId)
    ? unit
...
function UpdateSmallItem(panel, gold) {
  try {
    ...
    panel.SetHasClass(
      'CanBuy',
      GetRemainingPrice(panel.itemName, {}) <=
        (gold || PlayerTables.GetTableValue('gold', Game.GetLocalPlayerID())) || notpurchasable,
    );
    panel.SetHasClass('NotPurchasableItem', notpurchasable);
    if (ItemStocks[panel.itemName] != null) {
      var CurrentTime = Game.GetGameTime();
      var RemainingTime =
        ItemStocks[panel.itemName].current_cooldown -
        (CurrentTime - ItemStocks[panel.itemName].current_last_purchased_time);
      ...
      if (stock === 0 && RemainingTime > 0) {
        ...
        stockOverlay.style.clip =
          'radial(50% 50%, 0deg, ' +
          -((RemainingTime / ItemStocks[panel.itemName].current_cooldown) * 360) +
          'deg)';
    ...
    if (index > -1) SmallItems.splice(index, 1);
    else {
      index = SmallItemsAlwaysUpdated.indexOf(panel);
      if (index > -1) SmallItemsAlwaysUpdated.splice(index, 1);
    }
  }
}

function GetRemainingPrice(itemName, ItemCounter, baseItem) {
  if (ItemCounter[itemName] == null) ItemCounter[itemName] = 0;
  ItemCounter[itemName] = ItemCounter[itemName] + 1;

  var itemCount = GetItemCountInInventory(
...
function SetQuickbuyStickyItem(itemName) {
  ...
  var itemPanel = $.CreatePanel(
    ...
    'QuickBuyStickyButtonPanel_item_' + itemName,
  );
...
function MakeQuickbuyCheckItem(itemName, ItemCounter, ItemIndexer, sourceExpectedCount) {
  var RecipeData = ItemData[itemName].Recipe;
  if (ItemCounter[itemName] == null) ItemCounter[itemName] = 0;
  if (ItemIndexer[itemName] == null) ItemIndexer[itemName] = 0;
  ItemCounter[itemName] = ItemCounter[itemName] + 1;
  ItemIndexer[itemName] = ItemIndexer[itemName] + 1;
  var itemCount =
    GetItemCountInCourier(
      Players.GetPlayerHeroEntityIndex(Game.GetLocalPlayerID()),
      itemName,
      true,
    ) +
  ...
  if (
    itemCount < ItemCounter[itemName] ||
    (itemName === QuickBuyTarget && itemCount - (sourceExpectedCount - 1) < ItemCounter[itemName])
  ) {
    if (RecipeData != null && RecipeData.items != null) {
      _.each(RecipeData.items[1], function(childName) {
      ...
      $('#QuickBuyPanelItems').FindChildTraverse(
        'QuickBuyPanelItems_item_' + itemName + '_id_' + ItemIndexer[itemName],
      ) == null
    ) {
      var itemPanel = $.CreatePanel(
        'Panel',
...
function RemoveQuickbuyItemChildren(itemName, ItemIndexer, bIncrease) {
  var RecipeData = ItemData[itemName].Recipe;
  if (bIncrease) ItemIndexer[itemName] = (ItemIndexer[itemName] || 0) + 1;
...
function RemoveQuckbuyPanel(itemName, index) {
  var panel = $('#QuickBuyPanelItems').FindChildTraverse(
    'QuickBuyPanelItems_item_' + itemName + '_id_' + index,
  );
  if (panel != null) {
...
(function() {
  ...
  GameEvents.Subscribe('arena_team_changed_update', function() {
    var stockdata = PlayerTables.GetTableValue(
      'panorama_shop_data',
      'ItemStocks_team' + Players.GetTeam(Game.GetLocalPlayerID()),
    );
  ...
  $.RegisterEventHandler('DragDrop', $('#QuickBuyStickyButtonPanel'), function(
    panelId,
    draggedPanel,
  ) {
