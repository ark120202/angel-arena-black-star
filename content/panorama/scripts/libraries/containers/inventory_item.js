...
function UpdateItem() {
	...
	var isShop = m_Container && m_Container.IsShop != null && m_Container.IsShop();
	var stock = -1;
	var price = -1;
	if (isShop && m_Item !== -1) {
...
function OnMouseOut() {
	...
	if (handler) {
		...
		try {
			ret = handler({
				unit: unit,
				containerID: m_contID,
				itemID: m_Item,
				slot: m_slot,
				containerPanel: m_Container,
...
function ActivateItem(bypass) {
	...
	if (action !== 0) {
		GameEvents.SendCustomGameEventToServer("Containers_OnLeftClick", {
			unit: unit,
			contID: m_contID,
			itemID: m_Item,
			slot: m_slot
...
function RightClickItem() {
	...
	if (action === 1) {
		GameEvents.SendCustomGameEventToServer("Containers_OnRightClick", {
			unit: unit,
			contID: m_contID,
			itemID: m_Item,
			slot: m_slot
