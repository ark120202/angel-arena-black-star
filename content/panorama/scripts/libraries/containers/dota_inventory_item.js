...
function ActivateItem() {
	...
	if (m_Item == -1)
		return;
	CustomHooks.panorama_shop_show_item_if_open.call(Abilities.GetAbilityName(m_Item));
...
function RightClickItem() {
	...
	contextMenu.GetContentsPanel().Unit = m_QueryUnit;
	contextMenu.GetContentsPanel().BLoadLayout("file://{resources}/layout/custom_game/libraries/containers/dota_inventory_context_menu.xml", false, false);
}
...
function OnDragDrop(panelId, draggedPanel) {
	...
	if (fromCont == -1) {
		...
		GameEvents.SendCustomGameEventToServer("Containers_OnDragFrom", {
			unit: Players.GetLocalPlayerPortraitUnit(),
			contID: fromCont,
			itemID: draggedItem,
			fromSlot: draggedPanel.m_OriginalPanel.GetSlot(),
			toContID: -1,
			toSlot: m_ItemSlot
		});
	}
...
function OnDragStart(panelId, dragCallbacks) {
	if (m_Item == -1 || !Entities.IsControllableByPlayer(m_QueryUnit, Game.GetLocalPlayerID())) {
		return true;
	}
...
function OnDragEnd(panelId, draggedPanel) {
	...
	if (!IsInStash() && !draggedPanel.m_DragCompleted) {
		...
		for (var i = 0; i < mouseEntities.length; i++) {
			if (mouseEntities[i].accurateCollision) {
				entity = mouseEntities[i].entityIndex;
				break;
			}
		}
		GameEvents.SendCustomGameEventToServer("Containers_OnDragWorld", {
			unit: Players.GetLocalPlayerPortraitUnit(),
			contID: -1,
			itemID: m_Item,
			slot: m_ItemSlot,
