...
function OpenContainer(msg) {
  //$.Msg("OpenContainer -- ", msg);
	...
	if (!containerPanel) {
		...
		var layoutFile = PlayerTables.GetTableValue(idString, "layoutFile") || "file://{resources}/layout/custom_game/libraries/containers/container.xml";
	...
	if (abil !== -1) {
		...
		var fun = function() {
			...
			if (behaviors != CLICK_BEHAVIORS.DOTA_CLICK_BEHAVIOR_CAST) {
				GameEvents.SendCustomGameEventToServer("Containers_HideProxy", {
					abilID: abil
				});
				return;
			}
...
function CheckShop() {
	...
	if (sel !== lastSel) {
		GameEvents.SendCustomGameEventToServer("Containers_Select", {
			entity: sel
		});
		lastSel = sel;
	...
	if (sel >= 1 && (Entities.IsCreepHero(sel) || Entities.IsRealHero(sel))) {
		...
		if (oldShop !== shop) {
			GameEvents.SendCustomGameEventToServer("Containers_EntityShopRange", {
				unit: sel,
				shop: shop
			});
		}
...
(function() {
  ...
  //containerPanel = $.CreatePanel( "Panel", panel, "" );
  //containerPanel.BLoadLayout("file://{resources}/layout/custom_game/libraries/containers/container.xml", false, false);
