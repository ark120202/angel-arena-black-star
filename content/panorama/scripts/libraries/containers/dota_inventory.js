...
function UpdateInventory() {
	...
	for (var i = 0; i < DOTA_ITEM_STASH_MAX; ++i) {
		...
		//  if (Abilities.GetAbilityName(item) == "item_dummy")
...
function CreateInventoryPanels() {
	...
	for (var i = 0; i < DOTA_ITEM_STASH_MAX; ++i) {
		...
		inventoryPanel.BLoadLayout("file://{resources}/layout/custom_game/libraries/containers/dota_inventory_item.xml", false, false);
...
function StashGrabAll() {
	var slotsUsed = [];
	$.Each(m_InventoryPanels, function(p) {
		var baseUnit = p.GetUnit();
		var thisItem = p.GetItem();
		if (thisItem != -1 && p.GetSlot() > 5 && Entities.IsControllableByPlayer(baseUnit, Game.GetLocalPlayerID())) {
			var emptySlot = -1;
			for (var i = 0; i < 6; i++) {
				var tempItem = Entities.GetItemInSlot(baseUnit, i);
				if (Items.IsStackable(thisItem) && Abilities.GetAbilityName(thisItem) == Abilities.GetAbilityName(tempItem)) {
					emptySlot = i;
					break;
				} else if (tempItem == -1 && slotsUsed.indexOf(i) == -1) {
					slotsUsed.push(i);
					emptySlot = i;
					break;
				}
