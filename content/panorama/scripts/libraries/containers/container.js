...
//aabs
var Duration_SpawnTime = -1;
var Duration_Remaining = -1;
var Duration_Thinker;
...
function SetLayout(layout) {
	...
	for (var key in layout) {
		...
		for (var j = 0; j < layout[key]; j++) {
			...
			child.BLoadLayout("file://{resources}/layout/custom_game/libraries/containers/inventory_item.xml", false, false);
...
function ButtonPress(number, name) {
	...
	if (handler) {
		...
		try {
			ret = handler({
				unit: unit,
				containerID: contID,
				containerPanel: $.GetContextPanel(),
				buttonID: number,
				buttonName: name
			});
		} catch (err) {
	...
	if (action !== 0) {
		GameEvents.SendCustomGameEventToServer("Containers_OnButtonPressed", {
			unit: unit,
			contID: contID,
			button: parseInt(number)
		});
		return;
	}
}
...
function NewContainer(id) {
	...
	//aabs
	if (pt.Duration_SpawnTime != null && pt.Duration_Remaining != null && $("#ContDurationBarFilledPart") != null) {
		Duration_SpawnTime = pt.Duration_SpawnTime;
		Duration_Remaining = pt.Duration_Remaining;
		var f = function() {
			var remainingTime = Game.GetGameTime() - Duration_SpawnTime;
			$("#ContDurationBarFilledPart").style.height = (100 - ((remainingTime / Duration_Remaining) * 100)) + "%";
			Duration_Thinker = $.Schedule(0.2, f);
		};
		f();
	}
}
...
function CloseClicked() {
	...
	if (action !== 0) {
		GameEvents.SendCustomGameEventToServer("Containers_OnCloseClicked", {
			unit: unit,
			contID: contID
		});
		return;
