...
function OnDealDamage(keys)
	...
	local ability = keys.ability
	if ability:RollPRD(ability:GetLevelSpecialValueFor("bash_chance", ability:GetLevel() - 1), "bash_chance_PRD") and not caster:HasModifier("modifier_item_refresher_core_bash_cooldown") then
