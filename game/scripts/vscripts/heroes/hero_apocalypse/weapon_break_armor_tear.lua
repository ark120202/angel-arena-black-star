...
function ApplyStacks(keys)
	...
	local ability = keys.ability
	local stacks = target:GetPhysicalArmorBaseValue() * ability:GetAbilitySpecial("armor_cut_pct")
	if target.GetAgility then
		stacks = stacks + ((target:GetAgility() * DEFAULT_ARMOR_PER_AGI) * ability:GetAbilitySpecial("armor_cut_pct"))
	end
