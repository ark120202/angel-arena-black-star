...
if IsServer() then
	function guldan_shadow_path:OnSpellStart()
		...
		if caster:HasScepter() then
			caster:AddNewModifier(caster, self, "modifier_guldan_shadow_path", {duration = self:GetSpecialValueFor("duration_scepter")})
		else caster:AddNewModifier(caster, self, "modifier_guldan_shadow_path", {duration = self:GetSpecialValueFor("duration")})
		end
	end
	...
	function modifier_guldan_shadow_path:OnIntervalThink()
		...
		if not caster:HasScepter() then
			for _,v in ipairs(FindUnitsInRadius(target:GetTeamNumber(), target:GetAbsOrigin(), nil, self:GetAbility():GetSpecialValueFor('aura_radius'), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)) do
			...
			for _,v in ipairs(FindUnitsInRadius(target:GetTeamNumber(), target:GetAbsOrigin(), nil, self:GetAbility():GetSpecialValueFor('aura_radius_scepter'), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)) do
				local modifier = v:FindModifierByNameAndCaster("modifier_guldan_shadow_path_effect", target)
				if not modifier then
					v:AddNewModifier(target, ability, "modifier_guldan_shadow_path_effect", {duration = 0.11})
				else
