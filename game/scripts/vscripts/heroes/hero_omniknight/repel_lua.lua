...
LinkLuaModifier("modifier_omniknight_repel_lua", "heroes/hero_omniknight/modifiers/modifier_omniknight_repel_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_omniknight_repel_inverse_lua", "heroes/hero_omniknight/modifiers/modifier_omniknight_repel_inverse_lua", LUA_MODIFIER_MOTION_NONE)
function omniknight_repel_lua:OnSpellStart() if IsServer() then
	local targets = {}
	...
	for _,target in ipairs(targets) do
		...
		if target:GetTeam() == self:GetCaster():GetTeam() then
			...
			target:AddNewModifier(self:GetCaster(), self, "modifier_omniknight_repel_lua", {duration = self:GetSpecialValueFor("duration")})
		...
		elseif self:GetCaster():HasScepter() then
			...
			target:AddNewModifier(self:GetCaster(), self, "modifier_omniknight_repel_inverse_lua", {duration = self:GetSpecialValueFor("duration")})
			ParticleManager:CreateParticle("particles/units/heroes/hero_omniknight/omniknight_repel_cast.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
		end
	end
end end
...
function omniknight_repel_lua:CastFilterResultTarget(hTarget)
...
function omniknight_repel_lua:GetCustomCastErrorTarget(hTarget)
	if not self:GetCaster():HasScepter() and self:GetCaster():GetTeamNumber() ~= hTarget:GetTeamNumber() then
