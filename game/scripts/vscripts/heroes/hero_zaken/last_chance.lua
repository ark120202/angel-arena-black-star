function CheckHealthCrit(keys)
	...
	if (target:GetHealth() / target:GetMaxHealth()) * 100 <= ability:GetLevelSpecialValueFor("crit_health_pct", level) and ability:RollPRD(ability:GetLevelSpecialValueFor("crit_chance_pct", level), "crit_PRD") and ability:PerformPrecastActions() then
...
function CheckHealth(keys)
	...
	if ability:IsCooldownReady() and ability:IsOwnersManaEnough() and not target:IsMagicImmune() then
		if hppct <= ability:GetLevelSpecialValueFor("stun_health_pct", level) and ability:RollPRD(ability:GetLevelSpecialValueFor("stun_chance_pct", level), "stun_PRD") and ability:PerformPrecastActions() then
		...
		elseif hppct <= ability:GetLevelSpecialValueFor("root_health_pct", level) and ability:RollPRD(ability:GetLevelSpecialValueFor("root_chance_pct", level), "root_PRD") and ability:PerformPrecastActions() then
		...
		elseif hppct <= ability:GetLevelSpecialValueFor("silence_health_pct", level) and ability:RollPRD(ability:GetLevelSpecialValueFor("silence_chance_pct", level), "silence_PRD") and ability:PerformPrecastActions() then
