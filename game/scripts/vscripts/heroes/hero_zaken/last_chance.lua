function CheckHealthCrit(keys)
	...
	if target:GetHealthPercent() <= ability:GetLevelSpecialValueFor("crit_health_pct", level) and RollPercentage(ability:GetLevelSpecialValueFor("crit_chance_pct", level)) and PreformAbilityPrecastActions(caster, ability) then
...
function CheckHealth(keys)
	...
	if ability:IsCooldownReady() and ability:IsOwnersManaEnough() and not target:IsMagicImmune() then
		if target:GetHealthPercent() <= ability:GetLevelSpecialValueFor("stun_health_pct", level) and RollPercentage(ability:GetLevelSpecialValueFor("stun_chance_pct", level)) and PreformAbilityPrecastActions(caster, ability) then
		...
		elseif target:GetHealthPercent() <= ability:GetLevelSpecialValueFor("root_health_pct", level) and RollPercentage(ability:GetLevelSpecialValueFor("root_chance_pct", level)) and PreformAbilityPrecastActions(caster, ability) then
		...
		elseif target:GetHealthPercent() <= ability:GetLevelSpecialValueFor("silence_health_pct", level) and RollPercentage(ability:GetLevelSpecialValueFor("silence_chance_pct", level)) and PreformAbilityPrecastActions(caster, ability) then
