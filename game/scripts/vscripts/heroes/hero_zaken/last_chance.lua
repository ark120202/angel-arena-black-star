function CheckHealthCrit(keys)
	...
	if (target:GetHealth() / target:GetMaxHealth()) * 100 <= ability:GetLevelSpecialValueFor("crit_health_pct", level) and RollPercentage(ability:GetLevelSpecialValueFor("crit_chance_pct", level)) and PreformAbilityPrecastActions(caster, ability) then
...
function CheckHealth(keys)
	...
	if ability:IsCooldownReady() and ability:IsOwnersManaEnough() and not target:IsMagicImmune() then
		if hppct <= ability:GetLevelSpecialValueFor("stun_health_pct", level) and RollPercentage(ability:GetLevelSpecialValueFor("stun_chance_pct", level)) and PreformAbilityPrecastActions(caster, ability) then
		...
		elseif hppct <= ability:GetLevelSpecialValueFor("root_health_pct", level) and RollPercentage(ability:GetLevelSpecialValueFor("root_chance_pct", level)) and PreformAbilityPrecastActions(caster, ability) then
		...
		elseif hppct <= ability:GetLevelSpecialValueFor("silence_health_pct", level) and RollPercentage(ability:GetLevelSpecialValueFor("silence_chance_pct", level)) and PreformAbilityPrecastActions(caster, ability) then
