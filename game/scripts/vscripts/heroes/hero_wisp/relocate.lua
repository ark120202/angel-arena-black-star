...
function CastRelocate( event )
	...
	if caster:HasModifier( tetherModifierName ) then
		...
		for _,v in ipairs(tetheredAlly) do
			if IsRelocatableUnit( v ) then
				tetherAbility:ApplyDataDrivenModifier( caster, caster, tetherModifierName, {} )
				tetherAbility:ApplyDataDrivenModifier( caster, v, event.tether_ally_modifier, {} )
			end
		end
	end
...
function Teleportation_PreDestroyTree( event )
	...
	if caster:HasModifier( tetherModifierName ) then
		...
		for _,v in ipairs(tetheredAlly) do
			if IsRelocatableUnit( v ) then
				...
				tetherAbility:ApplyDataDrivenModifier( caster, v, event.tether_ally_modifier, { duration = tetherDuration } )
			else
				v = nil
			end
		end
	...
	if tetheredAlly then
		for _,v in ipairs(tetheredAlly) do
			pfx = ParticleManager:CreateParticle( event.teleport_particle, PATTACH_POINT, v )
			ParticleManager:SetParticleControlEnt( pfx, 0, v, PATTACH_POINT, "attach_hitloc", v:GetAbsOrigin(), true )
		end
	end
...
function Teleportation_PostDestroyTree( event )
	...
	if tetheredAlly then
		for _,v in ipairs(tetheredAlly) do
			FindClearSpaceForUnit( v, ability.relocate_targetPoint + Vector( 16, 16, 0 ), false )
			v:Stop()
		end
...
function ReturningTeleportation_PreDestroyTree( event )
	...
	if caster:HasModifier( event.tether_modifier ) then
		tetheredAlly = tetherAbility[event.tether_ally_property_name]
		for _,v in ipairs(tetheredAlly) do
			if not IsRelocatableUnit( v ) then
				tetheredAlly = nil
			end
		end
	end
...
function ReturningTeleportation_PostDestroyTree( event )
	...
	if tetheredAlly then
		for _,v in ipairs(tetheredAlly) do
			FindClearSpaceForUnit( v, ability.relocate_originalPoint + Vector( 16, 16, 0 ), false )
			v:Stop()
		end
		ability.relocate_tetheredAlly = nil
	end
