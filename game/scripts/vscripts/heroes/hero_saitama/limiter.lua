...
if IsServer() then
	...
	function modifier_saitama_limiter_autocast:OnIntervalThink()
		...
		if parent:IsAlive() then
			if ability:GetAutoCastState() and parent:GetMana() >= ability:GetManaCost() and not parent:IsChanneling() and not parent:IsInvisible() and (not parent:GetCurrentActiveAbility() or not parent:GetCurrentActiveAbility():IsInAbilityPhase()) and parent:GetModifierStackCount("modifier_saitama_limiter", parent) > 0 then
