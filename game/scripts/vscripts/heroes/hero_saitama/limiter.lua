...
if IsServer() then
	...
	function modifier_saitama_limiter_autocast:OnIntervalThink()
		...
		if parent:IsAlive() then
			if not parent:IsChanneling() and ability:GetAutoCastState() and parent:GetModifierStackCount("modifier_saitama_limiter", parent) > 0 and parent:GetMana() >= ability:GetManaCost() then
